...
public class PlayerVariables {
	...
	public static String replaceVariables(String string, Player player) {
		...
		if (newString.contains("<exp>")) {
			newString = newString.replaceAll("<exp>",
					String.valueOf((double) Math.round(player.getExp() * 100.0D) / 100));
		}
		...
		if (newString.contains("<hunger>")) {
			...
			if (newString.contains("<health>")) {
				newString = newString.replaceAll("<health>", String.valueOf((double) (player.getHealth())));
			}
			...
			if (newString.contains("<worldplayers>")) {
				...
				if (newString.contains("<helmet>")) {
					newString = newString.replaceAll("<helmet>",
							String.valueOf((player.getInventory().getHelmet() == null ? "None"
									: player.getInventory().getHelmet().getType().name()) + ""));
				}
				if (newString.contains("<helmetdurability>")) {
					newString = newString.replaceAll("<helmetdurability>",
							String.valueOf(player.getInventory().getHelmet() == null ? 0
									: player.getInventory().getHelmet().getDurability()));
				}
				if (newString.contains("<chestplate>")) {
					newString = newString.replaceAll("<chestplate>",
							String.valueOf((player.getInventory().getChestplate() == null ? "None"
									: player.getInventory().getChestplate().getType().name()) + ""));
				}
				if (newString.contains("<chestplatedurability>")) {
					newString = newString.replaceAll("<chestplatedurability>",
							String.valueOf(player.getInventory().getChestplate() == null ? 0
									: player.getInventory().getHelmet().getDurability()));
				}
				if (newString.contains("<leggings>")) {
					newString = newString.replaceAll("<leggings>",
							String.valueOf((player.getInventory().getLeggings() == null ? "None"
									: player.getInventory().getLeggings().getType().name()) + ""));
				}
				if (newString.contains("<leggingsdurability>")) {
					newString = newString.replaceAll("<leggingsdurability>",
							String.valueOf(player.getInventory().getLeggings() == null ? 0
									: player.getInventory().getHelmet().getDurability()));
				}
				if (newString.contains("<boots>")) {
					newString = newString.replaceAll("<boots>", String.valueOf((player.getInventory().getBoots() == null
							? "None" : player.getInventory().getBoots().getType().name()) + ""));
				}
				if (newString.contains("<bootsdurability>")) {
					newString = newString.replaceAll("<bootsdurability>",
							String.valueOf(player.getInventory().getBoots() == null ? 0
									: player.getInventory().getHelmet().getDurability()));
				}
				if (newString.contains("<hand>")) {
					newString = newString.replaceAll("<hand>", String.valueOf(
							(player.getItemInHand() == null ? "None" : player.getItemInHand().getType().name())));
				}
				if (newString.contains("<handdurability>")) {
					newString = newString.replaceAll("<handdurability>", String
							.valueOf(player.getItemInHand() == null ? 0 : player.getItemInHand().getDurability()));
				}
